...
page.on('console', (msg) => {
  ...
  if (corsPatterns.some((p) => p.test(text))) corsConsoleIssues.push(text);
});
...
page.on('requestfinished', (request) => {
  ...
  if (url.includes('/projects/') && url.includes('/tasks')) taskRequestUrls.push(url);
});
...
try {
  ...
  const { token, projectId } = await page.evaluate(async ({ api, fallbackProjectName }) => {
    const tokenValue = localStorage.getItem('taskflow_token');
    ...
    const createResponse = await fetch(`${api}/projects`, {
      ...
      body: JSON.stringify({
        ...
        description: 'Runtime validation project',
      }),
    });
    const createBody = await createResponse.json();

    return { token: tokenValue, projectId: createBody?.id };
  }, { api: apiUrl, fallbackProjectName: projectName });

  assert(Boolean(token), 'Missing auth token after login');
  assert(Boolean(projectId), 'Unable to resolve project id for task flow');
  ...
  assert(corsConsoleIssues.length === 0, `CORS-related issue(s): ${corsConsoleIssues.join(' | ')}`);

  console.log('✅ Runtime flow validation passed');
  console.log(`Registered user: ${email}`);
  console.log(`Project: ${projectName}`);
  console.log(`Task: ${taskTitle}`);
  console.log('Filtered tasks requests observed:');
  [...new Set(taskRequestUrls)].forEach((url) => console.log(`- ${url}`));

  if (consoleErrors.length > 0) {
    console.log('Browser console errors observed (non-CORS):');
