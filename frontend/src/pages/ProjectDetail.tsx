...
export function ProjectDetail() {
  ...
  if (projectError || !projectData) {
    ...
    return (
      <div className="text-center py-12 text-slate-800">
        ...
        <Link to="/projects" className="text-slate-600 hover:text-indigo-700 mt-4 inline-block">
  ...
  return (
    <div className="space-y-8 h-full flex flex-col text-slate-900">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            <Link to="/projects" className="inline-flex items-center text-sm text-slate-600 hover:text-indigo-700 mb-4 transition-colors">
              <ArrowLeft className="w-4 h-4 mr-1" />
              Projects
            </Link>
            <h1 className="text-3xl font-semibold tracking-tight text-slate-900 leading-tight">{project.name}</h1>
            {project.description && <p className="text-slate-600 mt-2 max-w-2xl">{project.description}</p>}
          </div>
          <div className="flex items-center gap-3">
            <Button onClick={handleOpenCreate} className="bg-slate-900 text-white hover:bg-slate-800 shadow-sm">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-600">Status totals</p>
          <div className="mt-2 grid grid-cols-3 gap-2 text-sm">
            <div className="rounded-md bg-slate-50 p-2 text-center border border-slate-100">
              <p className="text-slate-600">To Do</p>
              <p className="font-semibold text-slate-900">{statsData?.by_status?.todo ?? 0}</p>
            </div>
            <div className="rounded-md bg-slate-50 p-2 text-center border border-slate-100">
              <p className="text-slate-600">In Progress</p>
              <p className="font-semibold text-slate-900">{statsData?.by_status?.in_progress ?? 0}</p>
            </div>
            <div className="rounded-md bg-slate-50 p-2 text-center border border-slate-100">
              <p className="text-slate-600">Done</p>
              <p className="font-semibold text-slate-900">{statsData?.by_status?.done ?? 0}</p>
            </div>
          </div>
        </div>

        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-600">Top assignees</p>
          <div className="mt-2 space-y-1 text-sm">
            {topAssignees.length === 0 ? (
              <p className="text-slate-600">No assignee activity yet.</p>
            ) : (
              topAssignees.map((item) => (
                <div key={item.key} className="flex items-center justify-between rounded-md bg-slate-50 border border-slate-100 px-2 py-1">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-end sm:justify-between">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
          <label className="space-y-1">
            ...
            <select
              ...
              className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300 sm:min-w-44"
            >
          ...
          <label className="space-y-1">
            <span className="text-xs font-medium uppercase tracking-wide text-slate-600">Assignee</span>
            <select
              ...
              className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300 sm:min-w-56"
            >
        ...
        <div className="flex items-center justify-between gap-3 sm:justify-end">
          <p className="text-sm text-slate-600">
            Showing <span className="font-medium text-slate-900">{tasks.length}</span>
      ...
      <div className="flex-1 min-h-0 overflow-x-auto pb-4">
        {isTasksLoading ? (
          ...
          (() => {
            ...
            return (
              <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
                <p>{message}</p>
                {status && <p className="text-xs text-rose-700/80 mt-1">Error code: {status}</p>}
          ...
          <div className="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center">
            <p className="text-base font-medium text-slate-900">No tasks match current filters</p>
            <p className="mt-1 text-sm text-slate-600">Try another status/assignee combination or clear filters.</p>
          ...
          <>
            ...
            <div className="mt-4 flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
              <p className="text-sm text-slate-600">
                Page <span className="font-medium text-slate-900">{tasksMeta?.page ?? tasksPage}</span> · Total tasks{' '}
                <span className="font-medium text-slate-900">{tasksMeta?.total ?? tasks.length}</span>
                {isTasksFetching ? ' · Updating…' : ''}
              </p>
